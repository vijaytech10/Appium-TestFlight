...
if ! ps aux | grep -q "[a]ppium -a 0.0.0.0 -p 4723"; then
  ...
  appium -a 0.0.0.0 -p 4723 --default-capabilities '{"udid":"RZCT709AF3B"}' --relaxed-security --long-stacktrace --log-timestamp --local-timezone --log-level debug:debug --log /appium.log > /dev/null 2>&1 &
...
# Note: 'Device ID' update is required based on the connected test device i.e., qsyleufuizdepfnz
if ! /opt/homebrew/bin/adb devices | grep -q "RZCT709AF3B"; then
...
# Sample combination to be updated in file (i.e.) RZCT709AF3B::test_P0_dedicated_profile_sharing_cases_sender_01.py::TEST_ACCOUNT_SHARING_US_001::Android
# Test accounts to be updated based on configurations made in "/src/tree_top_alexa_social_tests/constants/test_accounts.py"
TEST_COMBINATIONS_FILE="$UTILITIES_DIR/test_input_combinations.txt"
